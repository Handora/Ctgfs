...
//
void fuseserver_getattr(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi) {
  struct stat st;
...
void fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr,
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    VERIFY(yfs->isfile(ino));
    ret = yfs->setattr(ino, attr, st);
    if (ret != yfs_client::OK) {
...
void fuseserver_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
  ...
  VERIFY(yfs->isfile(ino));
  printf("fuseserver_read read size %lu from offset %ld of ino[%016llx]\n",
         size, off, (yfs_client::inum)ino);
  std::string buf;
  yfs_client::status ret;
  ret = yfs->read((yfs_client::inum)ino, size, off, buf);
  if (ret != yfs_client::OK) {
...
void fuseserver_write(fuse_req_t req, fuse_ino_t ino, const char *buf,
  ...
  VERIFY(yfs->isfile(ino));
  printf("fuseserver_write write size %lu from offset %ld of ino[%016llx]\n",
         size, off, (yfs_client::inum)ino);

  yfs_client::status ret;
  ret = yfs->write((yfs_client::inum)ino, size, off, std::string(buf, size));
  if (ret != yfs_client::OK) {
    fuse_reply_err(req, ENOSYS);
    return;
  }
...
yfs_client::status fuseserver_createhelper(fuse_ino_t parent, const char *name,
  ...
  printf("fuseserver_create %s on parent dir %016lx\n", name, parent);
  yfs_client::status ret;
  yfs_client::inum inum;
  yfs_client::fileinfo info;
  ret = yfs->create((yfs_client::inum)parent, std::string(name), inum, info);
  if (ret != yfs_client::OK) {
    return ret;
  }
...
void fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name) {
  ...
  printf("fuseserver_lookup look up %s in the parent dir %016lx\n", name,
         parent);
  yfs_client::status ret;
  bool is_dir;
  ...
  yfs_client::inum inum;
  ret = yfs->lookup((yfs_client::inum)parent, name, inum, is_dir, finfo, dinfo);
  if (ret != yfs_client::OK) {
    if (ret == yfs_client::NOENT) {
      fuse_reply_err(req, ENOENT);
    } else {
...
void fuseserver_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
  ...
  ret = yfs->readdir(inum, size, off, files);
  if (ret != yfs_client::OK) {
    if (ret == yfs_client::NOTDIR) {
  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    dirbuf_add(&b, files[i].name.c_str(), files[i].inum);
  }
...
void fuseserver_open(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi) {
...
void fuseserver_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name,
  ...
  printf("fuseserver_mkdir %s on parent dir %016lx\n", name, parent);

  yfs_client::status ret;
  yfs_client::inum inum;
  yfs_client::dirinfo info;
  ret = yfs->mkdir((yfs_client::inum)parent, std::string(name), inum, info);
  if (ret != yfs_client::OK) {
    if (ret == yfs_client::EXIST) {
      fuse_reply_err(req, EEXIST);
    } else {
  ...
  e.attr.st_mtime = info.mtime;
  e.attr.st_ctime = info.ctime;
  printf("   getattr(%016lx) -> %lu %lu %lu\n", e.ino, info.atime, info.mtime,
         info.ctime);
  printf("fuseserver_mkdir finish\n");
  fuse_reply_entry(req, &e);
}
...
void fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name) {
  printf("fuseserver_unlink %s on parent dir %016lx\n", name, parent);
  yfs_client::status ret;
  ret = yfs->unlink(parent, std::string(name));
  if (ret != yfs_client::OK) {
    if (ret == yfs_client::NOENT) {
      fuse_reply_err(req, ENOENT);
    } else {
