  ...
  while (true) {
    ...
    case RELEASING:
      // TODO(Handora): HOW TO DO RELEASING MORE SEPHISITICLY
      {
        ...
        pthread_mutex_lock(&lock_m_);
        tprintf("Client %s get back from acquire rpc with lock %llu in state %s\n", id.c_str(), lid, print_state(lsm_[lid].state).c_str());
        ...
        if (r == lock_protocol::RETRY) {
          lsm_[lid].state = NONE;
          tprintf("Client %s get back from acquire rpc with lock %llu and need wait for retry\n", id.c_str(), lid);
          waiting_threads++;
...
lock_client_cache::release(lock_protocol::lockid_t lid)
{
  ...
  case LOCKED:
    {
      if (lsm_[lid].is_revoked) {
        ...
        pthread_mutex_lock(&lock_m_);
        tprintf("Client %s get release rpc back when release lock %llu in state %s\n", id.c_str(), lid, print_state(lsm_[lid].state).c_str());
  ...
  if (need_send) {
    ...
    pthread_mutex_lock(&lock_m_);
    tprintf("Client %s get back from acquire for need_send of release with lock %llu in state %s\n", id.c_str(), lid, print_state(lsm_[lid].state).c_str());
                                  ...
                                  int &)
{
  ...
  case FREE:
    {
      lsm_[lid].state = RELEASING;
      tprintf("Client %s get revoke rpc with lock %llu in FREE and start to release\n", id.c_str(), lid);
      pthread_mutex_unlock(&lock_m_);
      lu->dorelease(lid);
      ret = cl->call(lock_protocol::release, lid, id, r);
      ...
      pthread_mutex_lock(&lock_m_);
      tprintf("Client %s get release rpc for revoke back with lock %llu in state %s\n", id.c_str(), lid, print_state(lsm_[lid].state).c_str());
      if (lsm_[lid].state == RELEASING)
        lsm_[lid].state = NONE;
      break;
    }
  case LOCKED:
    {
      tprintf("Client %s is locking for lock %llu. so we set tags and wait for the task to be finished\n", id.c_str(), lid);
      lsm_[lid].is_revoked = true;
      break;
    }
  case ACQUIRING:
    ...
    case RELEASING:
      {
        ...
        pthread_mutex_lock(&lock_m_);
        tprintf("Client %s get back from acquire for retry with lock %llu in state %s\n", id.c_str(), lid, print_state(lsm_[lid].state).c_str());
