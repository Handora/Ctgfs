...
class PrefixTreeNode {
 public:
  ...
  virtual std::pair<int, PrefixTreeNode*> FindNearestDomainId(
      bool left = true) = 0;
  ...
  friend void MergeFileNodeToDirNode(PrefixTreeNodePtr file_node,
                                     PrefixTreeNodePtr dir_node);
  friend void SplitFileNodeToDirNode(PrefixTreeNodePtr origin_file_node,
                                     PrefixTreeNodePtr cur_file_node);
...
class PrefixTreeDirNode : public PrefixTreeNode {
 public:
  ...
  PrefixTreeDirNode(const std::string& path, const unsigned long long ino,
 ...
 protected:
  std::set<PrefixTreeNodePtr, bool (*)(PrefixTreeNodePtr, PrefixTreeNodePtr)>
      list_;
};
...
class PrefixTreeFileNode : public PrefixTreeNode {
 public:
  ...
  PrefixTreeFileNode(const std::string& path, const unsigned long long ino,
                     const PrefixTreeNodePtr parent);
  ~PrefixTreeFileNode() {}
  bool IsDir();
  void PushDownDomainTag();
