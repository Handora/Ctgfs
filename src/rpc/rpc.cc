...
rpcs::dispatch(djob_t *j)
{
	...
	if(h.clt_nonce){
		...
		stat = checkduplicate_and_update(h.clt_nonce, h.xid,
                                                 h.xid_rep, &b1, &sz1);
	  jsl_log(JSL_DBG_2, "rpcs::dispatch: checkduplicate_and_update return stat %d \
        for clt[%u], xid[%u], xid_rep[%u]\n", 
			   		stat, h.clt_nonce, h.xid, h.xid_rep);
    VERIFY(reply_window_[h.clt_nonce].size() != 0);
	} else {
...
void
rpcs::gc_reply_window(unsigned int clt_nonce, unsigned int xid_rep)
{
  std::list<reply_t> window = reply_window_[clt_nonce];
  std::list<reply_t>::iterator it;
  for (it = window.begin(); it != window.end(); ) {
    if (it->xid < xid_rep && window.size() != 1) {
      delete(it->buf);
      it = window.erase(it);
    } else {
      break;
  ...
  reply_window_[clt_nonce] = window;
  jsl_log(JSL_DBG_1, "rpcs::gc_reply_window: gc to window of %u\n", window.front().xid);
}
		...
		unsigned int xid_rep, char **b, int *sz)
{
  ...
  if (window.size() == 0) {
    // first reply
    reply_t reply(xid);
    window.push_back(reply);
    reply_window_[clt_nonce] = window;
    return NEW;
  }

  if (xid < window.front().xid) {
    gc_reply_window(clt_nonce, xid_rep);
    return FORGOTTEN;
  }
