...
TEST(ClientTest, Connect) {
  ...
  // add service
  int add_status =
      server.AddService(&(*fs_service_ptr), brpc::SERVER_DOESNT_OWN_SERVICE);
  ASSERT_EQ(0, add_status) << "add service error! status : " << add_status
                           << std::endl;
  ...
  ASSERT_EQ(0, stream_create_status)
      << "create stream fail! status : " << stream_create_status << std::endl;
  ...
  do {
    std::string buf_str =
        std::string(test_str.begin() + st, test_str.begin() + ed);
    butil::IOBuf buf;
